...
const createUserRole = async (req, res) => {
  try {
    ...
    // Fetch all roles of the user to check for duplicate role name
    const existingRoles = await UserRole.find({ userId: user });

    for (const role of existingRoles) {
      const decryptedRoleName = JSON.parse(decryptData(role.UserRolename));
    ...
    // Validate and encrypt status
    if (status !== true && status !== false) {
      return res
        .status(400)
        .json(errorResponse(400, "Invalid status. Only true or false are allowed.", false));
    }
    ...
    if (!role) {
      return res
        .status(500)
        .json(errorResponse(500, "Failed to create role", false));
    }
    ...
    try {
     const user=req.user
        ...
        const decryptedPackages = findData.map(roleItem => {
            return {
                UserRolename: JSON.parse(decryptData(roleItem.UserRolename)),
        ...
        return res.status(503).json(errorResponse(503, "Something went wrong", message.error, false));
    }
...
const getUserRoleById = async (req, res) => {
    try {
      ...
      const decryptedUserRolename = JSON.parse(decryptData(findData.UserRolename));
...
const updateUserRole = async (req, res) => {
  try {
    ...
    for (const role of existingRoles) {
      const decryptedRoleName = JSON.parse(decryptData(role.UserRolename));
      if (decryptedRoleName.toLowerCase() === UserRolename.toLowerCase()) {
        return res.status(400).json(errorResponse(400, "This role name already exists.", false));
      }
    }
    ...
    return res.status(200).json(successResponse(200, "Role updated successfully", "", true, updatedRole));
  } catch (error) {
  ...
  const deleteUserRole = async (req, res) => {
    try {
        ...
        return res.status(201).json(successResponse(201, "Remove successfully", "", true, ""));
  ...
  export{createUserRole,getUserRoleById,getUserRoles,updateUserRole,deleteUserRole}